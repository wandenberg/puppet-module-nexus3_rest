...
describe Nexus3::Config do
  ...
  describe :file_path do
    specify 'should retrieve path to Puppet\'s configuration directory from the API' do
      ...
      expect(Nexus3::Config.file_path).to eq('/puppet/is/somewhere/else/nexus3_rest.conf')
    end

    specify 'should cache the filename' do
      expect(Nexus3::Config.file_path).to be(Nexus3::Config.file_path)
    end
  end
  ...
  describe :read_config do
    specify 'should raise an error if file is not existing' do
      ...
      expect { Nexus3::Config.read_config }.to raise_error(Puppet::ParseError, /file not found/)
    end

    specify 'should raise an error if Nexus base url is missing' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'nexus_base_url'})
      expect { Nexus3::Config.read_config }.to raise_error(Puppet::ParseError, /must contain a value for key 'nexus_base_url'/)
    end

    specify 'should raise an error if admin username is missing' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'admin_username'})
      expect { Nexus3::Config.read_config }.to raise_error(Puppet::ParseError, /must contain a value for key 'admin_username'/)
    end

    specify 'should raise an error if admin password is missing' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'admin_password'})
      expect { Nexus3::Config.read_config }.to raise_error(Puppet::ParseError, /must contain a value for key 'admin_password'/)
    end
    ...
    specify 'should read admin username' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.read_config[:admin_username]).to eq(admin_username)
    end

    specify 'should read admin password' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.read_config[:admin_password]).to eq(admin_password)
    end

    specify 'should raise an error if can_delete_repositories flag is missing' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials.reject{|key,value| key == 'can_delete_repositories'})
      expect { Nexus3::Config.read_config }.to raise_error(Puppet::ParseError, /must contain a value for key 'can_delete_repositories'/)
    end
    ...
    specify 'should read can_delete_repositories flag (false case)' do
      ...
      expect(Nexus3::Config.read_config[:can_delete_repositories]).to be_falsey
    end
    ...
    specify 'should read can_delete_repositories flag (true case)' do
      ...
      expect(Nexus3::Config.read_config[:can_delete_repositories]).to be_truthy
    end

    specify 'should use default connection timeout if not specified' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.read_config[:connection_timeout]).to be(10)
    end
    ...
    specify 'should read connection timeout' do
      ...
      expect(Nexus3::Config.read_config[:connection_timeout]).to be(60)
    end
    ...
    specify 'should read connection open timeout' do
      ...
      expect(Nexus3::Config.read_config[:connection_open_timeout]).to be(60)
    end

    specify 'should use default health check retries if not specified' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.read_config[:health_check_retries]).to be(50)
    end
    ...
    specify 'should read health check retries' do
      ...
      expect(Nexus3::Config.read_config[:health_check_retries]).to be(100)
    end

    specify 'should use default health check timeout if not specified' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.read_config[:health_check_timeout]).to be(3)
    end
    ...
    specify 'should read health check timeout' do
      ...
      expect(Nexus3::Config.read_config[:health_check_timeout]).to be(10)
    end
  end
  ...
  describe :resolve do
    specify 'should add base url to absolute url' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.resolve('/foobar')).to eq("#{nexus_base_url}/foobar")
    end
    ...
    specify 'should not add base url if the url starts with http' do
      expect(Nexus3::Config.resolve('http://somewhere.else/foobar')).to eq('http://somewhere.else/foobar')
    end

    specify 'should not add base url if the url starts with https' do
      expect(Nexus3::Config.resolve('https://secure.net/foobar')).to eq('https://secure.net/foobar')
    end
  end
  ...
  describe :can_delete_repositories do
    specify 'should return false if can_delete_repositories configuration parameter is set to false' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials)
      expect(Nexus3::Config.can_delete_repositories).to be_falsey
    end

    specify 'should return true if can_delete_repositories configuration parameter is set to true' do
      allow(YAML).to receive(:load_file).and_return(base_url_and_credentials.merge({'can_delete_repositories' => true}))
      expect(Nexus3::Config.can_delete_repositories).to be_truthy
    end

...
describe type_class.provider(:ruby) do
  ...
  describe 'create' do
    ...
    it 'raise a human readable error message if the operation failed' do
      ...
      expect { instance.create }.to raise_error(Puppet::Error, %r{Error while creating nexus3_user example})
    end
  end
  ...
  describe 'flush' do
    ...
    it 'raise a human readable error message if the operation failed' do
      allow(Nexus3::API).to receive(:execute_script).and_raise('Operation failed')
      instance.mark_config_dirty
      expect { instance.flush }.to raise_error(Puppet::Error, %r{Error while updating nexus3_user example})
    end
  ...
  describe 'destroy' do
    ...
    it 'raise a human readable error message if the operation failed' do
      allow(Nexus3::API).to receive(:execute_script).and_raise('Operation failed')
      expect { instance.destroy }.to raise_error(Puppet::Error, %r{Error while deleting nexus3_user example})
    end
  end

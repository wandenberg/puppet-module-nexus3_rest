...
describe type_class.provider(:ruby) do
  ...
  describe 'instances' do
    ...
    specify 'should map each returned values to the correspondent property' do
      ...
      expect(instances[0].repositories).to eql(%w[repo-a repo-b])
  ...
  describe 'create' do
    ...
    it 'raise a human readable error message if the operation failed' do
      ...
      expect { instance.create }.to raise_error(Puppet::Error, %r{Error while creating nexus3_repository_group example})
    end
  end
  ...
  describe 'a maven2 repository group' do
    ...
    it 'execute a script to create the group' do
      script = <<~EOS
        def config = new org.sonatype.nexus.repository.config.Configuration()
  ...
  describe 'flush' do
    ...
    describe 'a docker repository group update' do
      ...
      it 'execute a script to update the group' do
        script = <<~EOS
          def config = repository.repositoryManager.get('example').getConfiguration()
          config.online = true
          def group = config.attributes('group')
    ...
    it 'raise a human readable error message if the operation failed' do
      allow(Nexus3::API).to receive(:execute_script).and_raise('Operation failed')
      instance.mark_config_dirty
      expect { instance.flush }.to raise_error(Puppet::Error, %r{Error while updating nexus3_repository_group example})
    end
    ...
    describe 'when some value has changed' do
      before(:each) { instance.repositories = %w[name1 name3] }
  ...
  describe 'destroy' do
    ...
    it 'raise a human readable error message if the operation failed' do
      allow(Nexus3::API).to receive(:execute_script).and_raise('Operation failed')
      expect { instance.destroy }.to raise_error(Puppet::Error, %r{Error while deleting nexus3_repository_group example})
    end
  end
